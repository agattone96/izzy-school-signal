(() => {
  ...
  function renderSettings() {
    ...
    return `<h2 class="section-title">Settings</h2><p class="section-lead">One place for app status, display preferences, installation, and recovery. These preferences stay in this browser.</p><section class="status-panel"><dl><dt>Calendar</dt><dd>${receipt ? escapeHtml(receipt.schoolYear) : "Not installed"}</dd><dt>Events</dt><dd>${receipt ? events().length : 0}</dd><dt>Last successful sync</dt><dd>${sync ? new Date(sync.at).toLocaleString() : "Not yet"}</dd><dt>Offline</dt><dd>${receipt ? "Ready" : "Not ready"}</dd></dl></section><div class="settings-list"><div class="setting toggle"><div><span class="label">High contrast</span><p>Strengthens secondary text and divider lines.</p></div><input type="checkbox" data-setting="highContrast" ${state.settings.highContrast ? "checked" : ""} aria-label="High contrast"></div><div class="setting toggle"><div><span class="label">Reduce motion</span><p>Disables nonessential transitions.</p></div><input type="checkbox" data-setting="reduceMotion" ${state.settings.reduceMotion ? "checked" : ""} aria-label="Reduce motion"></div><div class="setting"><span class="label">Install on iPhone</span><p>In Safari, tap Share, then Add to Home Screen. iOS requires this confirmation and it cannot be automated honestly.</p><div class="action-stack"><button class="button" data-action="sync">Run health check</button><a class="button" href="https://raw.githubusercontent.com/agattone96/izzy-school-signal/main/Izzy%20School%20Signal%20Installer.js" target="_blank" rel="noopener">Optional widget installer</a></div></div><div class="setting"><span class="label">Recovery</span><p>Restore the previous validated browser copy if the active local cache ever needs repair.</p><div class="action-stack"><button class="button" data-action="restore">Restore last saved calendar</button><button class="button danger" data-action="clear">Clear browser cache</button></div></div></div>`;
  }
