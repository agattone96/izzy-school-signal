...
async function handleCommandCenterAction(action,presentation){
  ...
  try{
    ...
    if(action==="restore-calendar"){const a=new Alert();a.title="Restore Last Saved Calendar?";a.message="The previous validated calendar will become active. You can run this action again to undo the restore.";a.addAction("Restore Calendar");a.addCancelAction("Cancel");if(await a.presentAlert()===-1)return showCommandCenter(await loadPresentation(),Object.assign({},route,{route:"settings",focus:"settings-calendar"}));const restored=await CalendarProvider.restoreLastKnownGoodCalendar();await showMessage("Calendar Restored",`${restored.schoolYear} is active. The displaced calendar remains available for undo.`);return showCommandCenter(await loadPresentation(),Object.assign({},route,{route:"settings",focus:"settings-calendar"}));}
